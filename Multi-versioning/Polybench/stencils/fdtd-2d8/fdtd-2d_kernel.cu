...
__global__ void kernel0(double *ey, double *hz, int ny, int tmax, int nx, int c0)
{
    ...
    for (int c1 = 8 * b0; c1 < nx - 1; c1 += 2048)
      if (nx >= t0 + c1 + 2)
        for (int c2 = 8 * b1; c2 < ny; c2 += 2048)
          if (ny >= t1 + c2 + 1)
...
__global__ void kernel1(double *ex, double *hz, int ny, int tmax, int nx, int c0)
{
    ...
    for (int c1 = 8 * b0; c1 < nx; c1 += 2048)
      if (nx >= t0 + c1 + 1)
        for (int c2 = 8 * b1; c2 < ny - 1; c2 += 2048)
          if (ny >= t1 + c2 + 2)
...
__global__ void kernel2(double *_fict_, double *ex, double *ey, double *hz, int ny, int tmax, int nx, int c0)
{
    ...
    __shared__ double shared_ey[9][8];
    ...
    #define ppcg_min(x,y)    ({ __typeof__(x) _x = (x); __typeof__(y) _y = (y); _x < _y ? _x : _y; })
    {
      ...
      if (ny >= 8 * b1 + 2)
        for (int c1 = 8 * b0; c1 < nx - 1; c1 += 2048) {
          for (int c2 = 8 * b1; c2 < ny - 1; c2 += 2048) {
            if (ny >= t1 + c2 + 1)
              for (int c3 = t0; c3 <= ppcg_min(8, nx - c1 - 1); c3 += 8)
          ...
          if (b0 == 0 && c1 == 0 && (-ny + 8 * b1 + 1) % 2048 == 0) {
            if (t1 == 0)
              for (int c3 = t0; c3 <= ppcg_min(8, nx - 1); c3 += 8)
