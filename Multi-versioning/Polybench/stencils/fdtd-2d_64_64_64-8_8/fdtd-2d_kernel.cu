...
__global__ void kernel0(double *ey, double *hz, int ny, int tmax, int nx, int c0)
{
    ...
    for (int c1 = 64 * b0; c1 < nx - 1; c1 += 16384)
      for (int c2 = 64 * b1; c2 < ny; c2 += 16384)
        for (int c3 = t0; c3 <= ppcg_min(63, nx - c1 - 2); c3 += 8)
          for (int c4 = t1; c4 <= ppcg_min(63, ny - c2 - 1); c4 += 8)
...
__global__ void kernel1(double *ex, double *hz, int ny, int tmax, int nx, int c0)
{
    ...
    for (int c1 = 64 * b0; c1 < nx; c1 += 16384)
      for (int c2 = 64 * b1; c2 < ny - 1; c2 += 16384)
        for (int c3 = t0; c3 <= ppcg_min(63, nx - c1 - 1); c3 += 8)
          for (int c4 = t1; c4 <= ppcg_min(63, ny - c2 - 2); c4 += 8)
          ...
          for (int c2 = 64 * b1; c2 < ny - 1; c2 += 16384)
            if (ny >= t1 + c2 + 1)
              for (int c3 = t0; c3 <= ppcg_min(63, nx - c1 - 2); c3 += 8) {
                ...
                if (b0 == 0 && t0 == 0 && c1 == 0 && c2 + 64 >= ny && c3 == 0 && (-ny + t1 + 1) % 8 == 0)
